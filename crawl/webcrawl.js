...
function collectInternalLinks($) {
      ...
      relativeLinks.each(function() {
          ...
          checkUrlExist(url , function(err  ,isExits){
              if(isExits)
              {
              }
              else{
                var webData  = new WebUrl();
                webData.url = url;
                webData.save(function(err , weburl){
                    if(err) return err;
                    var response = {};
...
{
    WebUrl.findOne({url:url} , function(err , urlExist){
    if(err) callback(err , null);
    callback(null,urlExist);
    });
}
